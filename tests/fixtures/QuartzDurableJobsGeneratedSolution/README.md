...
- Consolidated tests: `tests/MartiX.QuartzTestApp.Tests`
...
- `Orders`: `src/MartiX.QuartzTestApp.Orders` (no synchronous module dependency)
...
```text
dotnet run --project src/MartiX.QuartzTestApp.Migrator -- validate
dotnet run --project src/MartiX.QuartzTestApp.Migrator -- script
dotnet run --project src/MartiX.QuartzTestApp.Migrator -- apply
```
...
`MartiX.QuartzTestApp.Client` project is generated only from that contract.
