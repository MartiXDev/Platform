  ...
  const renderingNote =
    plan.preset === FULL_STACK_PRESET
      ? (() => {
          ...
          return `The selected Application UI provider is \`${plan.ui.provider}\` with the \`${plan.ui.renderingProfile}\` rendering profile. The UI remains an HTTP/OpenAPI client; ${plan.ui.provider === "blazor-webapp" ? `Blazor uses ${claims.serverRendering} rendering.` : `the ${plan.ui.provider} shell uses its provider-native rendering boundary.`} Public SEO is \`${claims.publicSeo}\`; private responses use \`${claims.privateCaching}\`.`;
        })()
      : "";
    ...
    const bodySource =
      body === undefined
        ...
        : body.required
          ...
          : `        if (${body.parameterName} is not null)
        {
            httpRequest.Content = JsonContent.Create(${body.parameterName}, options: JsonOptions);
        }`;
    const requestPolicy =
...
function uiEvidenceFiles(plan) {
  ...
  const renderingMechanism =
    plan.ui.provider === "blazor-webapp"
      ? claims.serverRendering
      : `provider-native-${plan.ui.renderingProfile}`;
  const renderingEvidence =
    claims.publicSeo === "indexable-public-routes-only"
      ? "Public routes use static SSR and may make an indexable SEO claim; authenticated routes remain Interactive Server and are private, no-store, and excluded from public caching."
      : plan.ui.provider === "blazor-webapp"
        ? "Blazor uses prerendered Interactive Server rendering for application workflows; this profile makes no public SEO or shared-cache claim."
        : `The ${plan.ui.provider} provider uses its provider-native rendering boundary; this profile makes no public SEO or shared-cache claim.`;
  return {
...
`],
  ...
  if (plan.ui.provider === "blazor-webapp") {
    ...
    files.set(`${root}/Platform/Session/session.ts`, uiSessionFile());
